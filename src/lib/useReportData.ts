...
import { getLatestAssessment } from './assessmentApi';
import type { AssessmentRecord } from './assessmentApi';
...
// --- Public types (part of context API) ---
...
export interface UseReportDataResult {
  assessment: AssessmentFormData | null;
  loading: boolean;
  error: string;
  contentError: string;
  goalText: string;
...
export function useReportData(): UseReportDataResult {
  ...
  useEffect(() => {
    const fetchLatestAssessment = async () => {
      ...
      let data: AssessmentRecord | null;
      try {
        data = await getLatestAssessment(session.user.id, session.user.email);
      } catch (fetchError) {
