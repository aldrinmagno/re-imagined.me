...
function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900">
      <nav className="sticky top-0 z-50 border-b border-slate-300/80 bg-white/95 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 gap-6">
            ...
            <div className="hidden md:flex items-center">
              <Link
                to="/sign-in"
                className="inline-flex items-center rounded-full border border-emerald-300 bg-emerald-500 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-400"
              >
                Join waitlist
              </Link>
            </div>
          ...
          {mobileMenuOpen && (
            <div className="md:hidden py-4 space-y-3 border-t border-slate-300 text-sm text-slate-800">
              {navigation.map((item) => {
                const isActive = location.pathname === item.to;
                return (
                  <Link
